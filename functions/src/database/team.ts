...
const getScheduleData = async function () {
    ...
    schedules.forEach((item) => {
        ...
        schedule.place.Id = item.child('place').child('Id').val();
        schedule.place.Name = item.child('place').child('Name').val();
        schedule.place.SpeakingName = item.child('place').child('SpeakingName').val();
...
const makeSchedules = async function () {
    ...
    let maxCount: number = places.length - ((places.length * maxUser) - users.length);
    ...
    schedules.forEach((schedule, idx_schedule) => {
        console.log('  place: [' + schedule.place.Id + ']  [' + schedule.place.Name + ']  [' + schedule.place.SpeakingName + ']');
        schedule.users.forEach((user, idx_user) => {
            console.log('    ' + idx_user + '  user: [' + user.Id + ']  [' + user.Name + ']  [' + user.SpeakingName + ']');
        });
    });
...
export const getTeams = functions
    .https.onRequest(async (request: functions.Request, response: functions.Response) => {
        ...
        console.log('day: [' + day + ']');
        const readNext = await admin.database().ref('/next/date').once('value');
        ...
        if (readNext.val() < now || schedules.length <= 0) {
            ...
            const nextDate = new Date(nowDate.getFullYear(), nowDate.getMonth(), nowDate.getDate() + addDay);
            ...
            schedules_check.forEach((schedule, idx_schedule) => {
                console.log('  place: [' + schedule.place.Id + ']  [' + schedule.place.Name + ']  [' + schedule.place.SpeakingName + ']');
                schedule.users
                .forEach((user, idx_user) => {
                    console.log('    ' + idx_user + '  user: [' + user.Id + ']  [' + user.Name + ']  [' + user.SpeakingName + ']');
                });
            });

            const nextCheck = await admin.database().ref('/next/date').once('value');
        ...
        response.status(200).json(schedules).end();
    });
